...
class cuisinefilter(APIView):
    def get(self,request):
        ...
        re=recipe.objects.filter(cuisine=query)
        r=recipeserializers(re,many=True)
        return Response(r.data)

class mealfilter(APIView):
    def get(self,request):
        query=self.request.query_params.get('mealtype')
        re=recipe.objects.filter(mealtype=query)
        r=recipeserializers(re,many=True)
        return Response(r.data)
...
class Ingredientfilter(APIView):
    def get(self,request):
        ...
        re=recipe.objects.filter(ingredients=query)
        r=recipeserializers(re,many=True)
        return Response(r.data)